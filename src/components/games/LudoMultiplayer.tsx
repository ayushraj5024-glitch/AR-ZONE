    ...
    <div className="p-4 lg:p-8 w-full max-w-400 mx-auto space-y-6 font-sans pb-16">
      ...
      <div>
        ...
        <div className="text-sm text-slate-400 mt-2 flex items-center space-x-2 font-medium">
          ...
          <span className="text-\(--primary\)">Live Casino</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        {games.map((game) => (
          <div
            ...
            className="group relative bg-[#05100a] rounded-2xl overflow-hidden shadow-[0_0_15px_rgba(0,255,136,0.05)] hover:shadow-[0_0_30px_rgba(0,255,136,0.15)] transition-all duration-300 border border-\(--primary\)/20 hover:border-\(--primary\) cursor-pointer animate-in fade-in"
            onClick={() => onSelectGame(game.id, game.title)}
          >
            {/* Image container with gradient overlay */}
            <div className="relative aspect-4/3 w-full overflow-hidden bg-slate-900 border-b border-\(--primary\)/20">
              <img
                src={game.image}
                alt={game.title}
              ...
              >
                <div className="bg-\(--primary\)/90 text-[#05100a] p-4 rounded-full shadow-[0_0_20px_rgba(0,255,136,0.5)] transform transition-transform duration-300 group-hover:scale-110">
            ...
            <div className="p-4 bg-[#05100a]">
              ...
              <p className="text-\(--primary\) text-xs font-bold uppercase tracking-widest mt-1 opacity-0 group-hover:opacity-100 transition-opacity duration-300 transform translate-y-2 group-hover:translate-y-0">
                Play Now
              </p>
            </div>
          </div>
